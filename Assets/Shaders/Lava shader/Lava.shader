...
Shader "Lava" {
    Properties {
        ...
        _TextureDistanse ("TextureDistanse", Range(0, 1)) = 0.2869565
        _FresnelStrench ("FresnelStrench", Float ) = 1
        _TextureStrench ("TextureStrench", Range(-1, 1)) = 0.6633663
        _UndoEffectTexture ("UndoEffectTexture", 2D) = "bump" {}
        [MaterialToggle] _Fresnel ("Fresnel", Float ) = 0
        _EmmisionStrench ("EmmisionStrench", Range(0, 8)) = 1.8
        _Relief ("Relief", Range(0, 2)) = 0
...
////// Emissive:
                ...
                float2 UV = i.uv0;
                float2 node_4937 = UV;
                float3 _UndoEffectTexture_var = UnpackNormal(tex2D(_UndoEffectTexture,TRANSFORM_TEX(node_4937, _UndoEffectTexture)));
